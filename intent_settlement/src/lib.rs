...
pub enum DataKey {
    ...
    /// **Instance storage.** The SAC/SEP-41 token `Address` used for solver
    /// bonds and slash payments (typically USDC on Stellar).  Written once
    /// by `initialize` and never changed thereafter.
    BondToken,

    /// **Persistent storage.** One `IntentRecord` per submitted intent,
    /// keyed by the 32-byte deterministic `intent_id`.  Created by
    /// `submit_intent` and updated by `accept_intent`, `fill_intent`,
    /// `cancel_intent`, `slash_solver`, and `expire_intent`.  The TTL is
    /// bumped on every write; entries persist until the TTL lapses
    /// (threshold: 14 days, extended to 30 days) or are explicitly removed.
    Intent(BytesN<32>),

    /// **Persistent storage.** One `SolverRecord` per registered solver,
    /// keyed by the solver's `Address`.  Created by `register_solver` (first
    /// call), updated by `register_solver` (top-up), `withdraw_bond`,
    /// `accept_intent`, `fill_intent`, `slash_solver`, and removed by
    /// `deregister_solver`.  TTL is bumped on every write (same thresholds
    /// as `Intent`).
    Solver(Address),

    /// **Instance storage.** Running count of all intents ever submitted
    /// (`u64`).  Incremented by `submit_intent`.  Never decremented.
    TotalIntents,
